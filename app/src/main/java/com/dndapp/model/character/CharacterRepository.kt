...
class CharacterRepository(
    ...
    private val transformation: (input: JoinedCharacter) -> Character = {
        ...
        Character(it.character.name, race, background, characterClass, it.character.id)
    }
    ...
    fun add(character: Character) {
        character.apply {
            characterDao.insert(CharacterEntity(name, race.id, background.id, characterClass.id))
        }
    }
}